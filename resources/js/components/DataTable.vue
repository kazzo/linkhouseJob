        ...
        <div class="row justify-content-center">
            <div class="col-md-12">
                ...
                <div class="form-group row my-2">
                        <label for="szukaj" class="col-sm-2 col-form-label">Szukaj nazwy lub loginu</label>
                        <div class="col-sm-10"><input class="form-control" id="szukaj" @input="filter($event)" v-model="szukaj" ref="searchInput" /></div>
                </div>
                    ...
                    <table class="table table-striped">
                        <thead class="table-dark">
                            <tr>                                
                                ...
                                ><span v-show="sort_column===column"><span v-show="sort_direction==='up'">&#x25B2;</span><span v-show="sort_direction==='down'">&#x25BC;</span></span><span>{{ columnTitle(column) }}</span>                              
                        ...
                        <tbody>
                            <tr v-if="tableData.length === 0">
                                <td class="text-center" :colspan="columns.length + 1"><div  class="alert alert-primary" role="alert">{{ table_message }}</div></td>
                            </tr>
                            <tr v-for="(data, key1) in tableData" :key="data.id" class="datatable-row" v-else>
                                <td v-for="(value, key) in data">{{ value }}</td>
                                <td class="text-center"><button v-if="data.id!==''" type="button" @click="sendID(data.id);" class="btn btn-primary">Wyślij</button></td>
                            </tr>
                        </tbody>
                    ...
                    <div class="form-group row">
                        <div class="col-md-4">
                            <div class="m-2 text-center row">
                                <label for="pagination" class="col-form-label col-md-6">Strona</label>
                                <div class="col-md-6">                                 
                                    <select id="pagination" @change="changePage($event)" v-model="page" class="form-control">
                                        <option v-for="index in num_pages" v-bind:value="index">{{ index }}</option>
                                    </select>
                                </div>
                            </div>
                        </div>    
                        <div class="col-md-8 text-right">
                            <!-- div class="float-right" -->
                                <button type="button" id="reset" @click="getData();" class="btn btn-primary m-2">RESET</button>
                                <button type="button" id="change-api-key" @click="setApiKey();" class="btn btn-primary m-2">Zmień klucz API</button>
                            <!-- /div -->    
                        </div>
                    </div>                            
                </div>
    ...
    export default {
        mounted() {
            ...
            window.addEventListener("keypress", e => {
        	   if(this.$refs.searchInput !== document.activeElement) {
                   this.$refs.searchInput.value += String.fromCharCode(e.which);
        ...
        data() {
            return {
                ...
                sort_direction: '',
                page: 1,
                table_message: 'Brak danych do wyświetlenia',
            };
        },
